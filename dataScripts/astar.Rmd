...
time_period <- 0.01
curve_points$distance <- find_dist_from_df(curve_points)
curve_points$speed <- find_speed_from_distance(curve_points$distance, time_period)
curve_points$acceleration <- differential_with_time(curve_points$speed, time_period)
curve_points$jerk <- differential_with_time(curve_points$acceleration, time_period)
curve_points$time <- (0:(nrow(curve_points) - 1)) * time_period
```

visualization:
```{r speed_acc_jerk, warning=FALSE}
curve_points <- curve_points %>% as.data.frame()
ggplot(data = curve_points, mapping = aes(x = time, y = speed)) +
  geom_path() +
  geom_smooth() 
ggplot(data = curve_points, mapping = aes(x = time, y = acceleration)) + 
  geom_path()  +
  geom_smooth() 
ggplot(data = curve_points, mapping = aes(x = time, y = jerk)) + 
  geom_path()  +
  geom_smooth() 
```